...
def similarity(u, v, userRatings):
    ...
    # creates a list of movies that both u and v have rated 
...
def kNearestNeighbors(u, userRatings, k):
    # find similarity between u and all other users 
...
# this will be used to find the mean RMSE for each call to CFR and CFMMR
meanCFRatingsMean = []
randomRatingsMean = []
meanMovieRatingsMean = []
meanUserRatingsMean = []
meanRatingsMean = []
meanCFMMRatingsMean = []
meanCFMMRatingsMean25 = []
meanCFRatingsMean25 = []
meanCFMMRatingsMean300 = []
meanCFRatingsMean300 = []
meanCFRatingsMean500 = []
meanCFMMRatingsMean500 = []
meanCFRatingsMeanAll = []
meanCFMMRatingsMeanAll = []
...
for y in range(10):
    ...
    # these lists will hold each of the 14 calls to CF and CFMMR 0-numUsers
    meanMovieRatings = []
    randomRatings = []
    meanUserRatings = []
    meanRatings = []
    ...
    meanCFRatingsAll = []
    meanCFMMRatingsAll = []
    # friends list will hold all similar users to user u
    friendsList = [kNearesetNeighbors(x, trUserRatings,numUsers) for x in range(943)]
    
        
        #mean CFR, CFMMR ratings with k = 0
    ...
    meanCFMMRatings = [CFMMRatingPrediction(x[0],x[1],trUserRatings,trMovieRatings,[]) for x in testingSet]
        ...
        #mean CFR, CFMMR ratings with k = 25
    ...
    meanCFMMRatings25 = [CFMMRatingPrediction(x[0],x[1],trUserRatings,trMovieRatings,friendsList[x[0]-1][:25]) for x in testingSet]
        ...
        #mean CFR, CFMMR ratings with k = 300
    ...
    meanCFMMRatings300 = [CFMMRatingPrediction(x[0],x[1],trUserRatings,trMovieRatings,friendsList[x[0]-1][:300]) for x in testingSet]
        ...
        #mean CFR, CFMMR ratings with k = 500
    ...
    meanCFMMRatings500 = [CFMMRatingPrediction(x[0],x[1],trUserRatings,trMovieRatings,friendsList[x[0]-1][:500]) for x in testingSet]
        ...
        # mean CFR, CFMMR ratings with k numUsers
    ...
    meanCFMMRatingsAll = [CFMMRatingPrediction(x[0],x[1],trUserRatings,trMovieRatings,friendsList[x[0]-1]) for x in testingSet]
        # four orginal prediction algorithms from phase 1
    ...
    #mean CF,CFMMR ratings with k = 0
    meanCFRatingsMean.append(rmse(actualRatings,meanCFRatings))
    meanCFMMRatingsMean.append(rmse(actualRatings,meanCFMMRatings))
    #mean CF,CFMMR ratings with k = 25   
    meanCFRatingsMean25.append(rmse(actualRatings,meanCFRatings25))
    meanCFMMRatingsMean25.append(rmse(actualRatings,meanCFMMRatings25))
    #mean CF,CFMMR ratings with k = 300   
    meanCFRatingsMean300.append(rmse(actualRatings,meanCFRatings300))
    meanCFMMRatingsMean300.append(rmse(actualRatings,meanCFMMRatings300))
    #mean CF,CFMMR ratings with k = 500    
    meanCFRatingsMean500.append(rmse(actualRatings,meanCFRatings500))
    meanCFMMRatingsMean500.append(rmse(actualRatings,meanCFMMRatings500))
    #mean CF,CFMMR ratings with k = numUsers   
    meanCFRatingsMeanAll.append(rmse(actualRatings,meanCFRatingsAll))
    meanCFMMRatingsMeanAll.apend(rmse(actualRatings,meanCFMMRatingsAll))
    # four original prediction from phase 1    
    randomRatingsMean.append(rmse(actualRatings,randomRatings))
    meanMovieRatingsMean.append(rmse(actualRatings,meanMovieRatings))
    meanUserRatingsMean.append(rmse(actualRatings,meanUserRatings))
    meanRatingsMean.append(rmse(actualRatings,meanRatings))
    print(y+1,"iterations done")
                         

# Compute RMSE for each algorithm and produce output
print("Random Prediction RMSE:", sum(randomRatingsMean)/10)
print("Simple User-based Mean Prediction RMSE:", sum(meanUserRatingsMean)/10)
print("Simple Movie-based Mean Prediction RMSE:", sum(meanMovieRatings)/10)
print("Simple User-Movie-based Mean Prediction RMSE:", sum(meanRatingsMean)/10)
print("Collaborative filtering RMSE 0: ", sum(meanCFRatingsMean)/10)
print("Collaborative filtering RMSE 0 with mm: ", sum(meanCFMMRatingsMean)/10)
print("Collaborative filtering RMSE 25: ", sum(meanCFRatingsMean25)/10)
print("Collaborative filtering RMSE 25 with mm: ", sum(meanCFMMRatingsMean25)/10)
print("Collaborative filtering RMSE 300: ", sum(meanCFRatingsMean300)/10)
print("Collaborative filtering RMSE 300 with mm: ", sum(meanCFMMRatingsMean300)/10)
print("Collaborative filtering RMSE 500: ", sum(meanCFRatingsMean500)/10)
print("Collaborative filtering RMSE 500 with mm: ", sum(meanCFMMRatingsMean500)/10)
print("Collaborative filtering RMSE all: ", sum(meanCFRatingsMeanAll)/10)
print("Collaborative filtering RMSE all with mm: ", sum(meanCFMMRatingsMeanAll)/10)
